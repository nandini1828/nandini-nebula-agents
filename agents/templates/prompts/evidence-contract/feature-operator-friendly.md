...
- `G8 PM CLOSEOUT` — MUST switch role: read `agents/product-manager/SKILL.md` before executing (see closeout checklist below); VERIFY (not re-author) the G7 graph; finalize manifest, patch prior manifest, write `latest-run.json`, run `--stage closeout` exit 0
...
- `python3 agents/product-manager/scripts/validate-trackers.py` (calls feature-evidence at --stage G6 per §22; pass --feature {FEATURE_ID} --run-id {RUN_ID} when scoped)
